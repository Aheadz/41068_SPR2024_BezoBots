                                 ...
                                 map_height_(800), scale_(50.0)
{
    ...
    cv::resizeWindow("Robot Path Visualization", 400, 400);
...
void NavigateRobot::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
    ...
    cv::flip(map_image_, flip_image_, 0); // flipCode = 0 for vertical flip
    // Show the map image in the OpenCV window
    cv::imshow("Robot Path Visualization", flip_image_);
    cv::waitKey(1);
}
...
void NavigateRobot::odometryCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
    ...
    if (goal_index_ > 0 && goal_index_ < 2)
    {
        ...
        if (!keep_moving_shelf_)
        {
            ...
            shelf_movement_future_ = std::async(std::launch::async, [this]()
                ...
                while (keep_moving_shelf_)
                {
                    ...
                    shelf_x = robot_current_x_;
                    shelf_y = robot_current_y_+ 0.1;
                    shelf_z = robot_current_z_ + 1.0;

                    // Move the shelf to the updated position
                    moveShelf("Robo_Shelves_15", shelf_x, shelf_y, shelf_z);
                    ...
                    if (std::abs(shelf_x - target_shelf_x) <= tolerance && std::abs(shelf_y - target_shelf_y) <= tolerance) {
                        ...
                        if (goal_index_ > 1) {
                            moveShelf("Robo_Shelves_15", shelf_x, shelf_y, 0.043);  // Final z-coordinate
                            RCLCPP_INFO(this->get_logger(), "Shelf 15 reached final goal location. Stopping movement.");
    ...
    if (goal_index_ > 1) // After goal 2, ensure the shelf doesn't move anymore
    {
        if (keep_moving_shelf_)
        {
            // RCLCPP_INFO(this->get_logger(), "Stopping shelf movement after goal 2.");
            keep_moving_shelf_ = false; // Stop the shelf movement flag
        }
    }

    if (goal_index_ > 2 && goal_index_ < 4)
    {
        // RCLCPP_INFO(this->get_logger(), "Current goal (inside loop) %d ", goal_index_);
        //  Start the shelf movement asynchronously if not already running
        if (!keep_moving_shelf_2_)
        {
            // RCLCPP_INFO(this->get_logger(), "Pass keep moving condition");
            keep_moving_shelf_2_ = true;

            // Launch a future to handle the shelf movement asynchronously
            shelf_movement_future_ = std::async(std::launch::async, [this]()
                                                {
                while (keep_moving_shelf_2_)
                {
                    double target_shelf_x = 0.0, target_shelf_y = 0.0;

                    // Set target positions for the shelf based on the current goal
                    if (goal_index_ > 4) {
                        target_shelf_x = pose6.position.x;
                        target_shelf_y = pose6.position.y;
                    } 
                    ...
                    if (std::abs(shelf_x - target_shelf_x) <= tolerance && std::abs(shelf_y - target_shelf_y) <= tolerance) {
                        ...
                        if (goal_index_ > 4) {
                            ...
                            RCLCPP_INFO(this->get_logger(), "Shelf 11 reached final goal location. Stopping movement.");
                            keep_moving_shelf_2_ = false;  // Stop the shelf movement
    ...
    if (goal_index_ > 4) // After goal 4, ensure the shelf doesn't move anymore
    {
        if (keep_moving_shelf_2_)
        {
            RCLCPP_INFO(this->get_logger(), "Stopping shelf movement after goal 4.");
            keep_moving_shelf_2_ = false; // Stop the shelf movement flag
        }
    }

    if (goal_index_ > 5 && goal_index_ < 7)
    {
        // RCLCPP_INFO(this->get_logger(), "Current goal (inside loop) %d ", goal_index_);
        //  Start the shelf movement asynchronously if not already running
        if (!keep_moving_shelf_3_)
        {
            // RCLCPP_INFO(this->get_logger(), "Pass keep moving condition");
            keep_moving_shelf_3_ = true;

            // Launch a future to handle the shelf movement asynchronously
            shelf_movement_future_ = std::async(std::launch::async, [this]()
                                                {
                while (keep_moving_shelf_3_)
                {
                    double target_shelf_x = 0.0, target_shelf_y = 0.0;

                    // Set target positions for the shelf based on the current goal
                    if (goal_index_ > 6) {
                        target_shelf_x = pose7.position.x;
                        target_shelf_y = pose7.position.y;
                    } 
                    ...
                    // Move the shelf to the updated position
                    moveShelf("Robo_Shelves_3", shelf_x, shelf_y, shelf_z);
                    ...
                    if (std::abs(shelf_x - target_shelf_x) <= tolerance && std::abs(shelf_y - target_shelf_y) <= tolerance) {
                        ...
                        // Final goal (goal 4) reached, stop moving the shelf and lower it
                        if (goal_index_ > 6) {
                            moveShelf("Robo_Shelves_3", shelf_x, shelf_y, 0.043);  // Final z-coordinate
                            RCLCPP_INFO(this->get_logger(), "Shelf 3 reached final goal location. Stopping movement.");
                            keep_moving_shelf_3_ = false;  // Stop the shelf movement
    ...
    if (goal_index_ > 6) // After goal 4, ensure the shelf doesn't move anymore
    {
        if (keep_moving_shelf_3_)
        {
            RCLCPP_INFO(this->get_logger(), "Stopping shelf movement after goal 4.");
            keep_moving_shelf_3_ = false; // Stop the shelf movement flag
        }
    }
...
void NavigateRobot::sendGoal(int goal_index)
{
    ...
    // Shelf 15 spawned position
    pose1.position.x = 0.017;
    pose1.position.y = 6.7822;
    pose1.position.z = 0.042215; // First goal
    // Shelf 15 drop-off location
    pose2.position.x = 3.0;
    pose2.position.y = 7.8;
    pose2.position.z = 0.042215; // Second goal

    // Shelf 11 spawned position
    pose3.position.x = -0.044;
    pose3.position.y = 1.31;
    pose3.position.z = 0.042215;
    // Shelf 11 mid goal
    pose4.position.x = 1.5;
    pose4.position.y = -0.25;
    pose4.position.z = 0.042215;
    // Shelf 11 drop-off location
    pose5.position.x = 3.5;
    pose5.position.y = -0.25;
    pose5.position.z = 0.042215;

    // Shelf 3 spawned position
    pose6.position.x = 0.000134;
    pose6.position.y = -5.149418;
    pose6.position.z = 0.042215;
    // Shelf 3 drop-off location
    pose7.position.x = 3.0;
    pose7.position.y = -4;
    pose7.position.z = 0.042215; //

    // Robot returns to its intial position
    pose8.position.x = -2.007223;
    pose8.position.y = -0.500079;
    pose8.position.z = 0.042215; //
    ...
    std::vector<std::tuple<double, double, double>> goals = {
        ...
        {pose7.position.x, pose7.position.y, pose7.position.z},
        {pose8.position.x, pose8.position.y, pose8.position.z}};
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Distance to goal %d = %f (m)", goal_index_ + 1, feedback->distance_remaining);
    };
    ...
    send_goal_options.result_callback = [this](const auto &result)
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "Goal %d aborted. Attempting re-localization...", goal_index_ + 1);
            rclcpp::sleep_for(std::chrono::seconds(2)); // Wait for re-localization
            sendGoal(goal_index_);                      // Retry the same goal
        }
        else
        {
