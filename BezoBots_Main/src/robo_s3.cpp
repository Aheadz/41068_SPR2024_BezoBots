...
Sprint3::Sprint3() : Node("robo_s3"), map_received_(false), cylinder_diameter_(0.3), tf_buffer_(this->get_clock()), tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    odom_subscriber_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/odom", 10, std::bind(&Sprint3::odomCallback, this, std::placeholders::_1));
}
...
void Sprint3::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
    map_msg_ = *msg;
    map_received_ = true;
...
cv::Mat Sprint3::occupancyGridToImage(const nav_msgs::msg::OccupancyGrid::SharedPtr &grid)
{
...
void Sprint3::scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
    if (!map_received_)
    {
        return;
...
void Sprint3::checknewPoints(std::vector<std::pair<double, double>> circle_points)
{
...
std::vector<std::pair<double, double>> Sprint3::detectCircleClusters(std::vector<std::pair<double, double>> points)
{
...
void Sprint3::detectArc(const std::vector<std::pair<double, double>> &cluster, std::vector<std::pair<double, double>> &circle_centers)
{
    if (cluster.size() < 5)
    {
...
bool Sprint3::calculateCircleFromThreePoints(const std::pair<double, double> &p1,
...
void Sprint3::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
    if (!map_received_)
    {
        return;
...
void Sprint3::drawOnMap(double x, double y)
{
...
void Sprint3::convertToMapFrame(std::vector<std::pair<double, double>> &points)
{
