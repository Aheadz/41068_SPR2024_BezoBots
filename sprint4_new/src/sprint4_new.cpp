...
public:
    ...
    CylinderDetector() : Node("cylinder_detector"), cylinder_diameter_(0.3), circumnavigation_active_(false), move_to_point_c_(false), drive_around_cylinder_(false)
    {
        ...
        waitForActionServer();
        marker_publisher_ = this->create_publisher<visualization_msgs::msg::Marker>("detected_cylinder_markers", 10);
    }
...
private:
    ...
    void publishToOccupancyGrid(double centre_x, double centre_y)
    {
        ...
        // Publish a marker to visualize in RViz
        visualization_msgs::msg::Marker marker;
        marker.header.frame_id = "map";
        marker.header.stamp = this->get_clock()->now();
        marker.ns = "detected_cylinders";
        marker.id = detected_circles_.size(); // Unique ID for each marker
        marker.type = visualization_msgs::msg::Marker::CYLINDER;
        marker.action = visualization_msgs::msg::Marker::ADD;
        marker.pose.position.x = centre_x;
        marker.pose.position.y = centre_y;
        marker.pose.position.z = 0.5;        // Adjust this based on cylinder height
        marker.scale.x = cylinder_diameter_; // Diameter in the x direction
        marker.scale.y = cylinder_diameter_; // Diameter in the y direction
        marker.scale.z = 1.0;                // Height of the cylinder marker
